...
<head>
	...
	<script type="text/javascript">
		...
		function drawChart() {
			...
			if (flag==false) {document.getElementById("chart_div").innerHTML="<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; left: 2em; padding: 4px;color:red;'><b>Não existem dados para mostrar!</b></div></div>";
				return
			}
		...
		function drawChart1() {
			...
			for(i=0;i< t.length-1;i++) if (t[i]!=0) flag=true;
			if (flag == false) {
				document.getElementById("chart_div1").innerHTML = "<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; left: 2em; padding: 4px;color:red;'><b>Não existem dados para mostrar!</b></div></div>";
				return
			}
			var data = new google.visualization.DataTable();
			data.addColumn('string', 'Instituições');
		...
		function drawChart2() {
			...
			var flag=false;
			for(i=0;i< t.length-1;i++) if (t[i]!=0) flag=true;
				if (flag==false) {document.getElementById("chart_div2").innerHTML="<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; left: 2em; padding: 4px;color:red;'><b>Não existem dados para mostrar!</b></div></div>";return}
			var data = new google.visualization.DataTable();
			data.addColumn('string', 'Instituições');
			data.addColumn('number', 'Totais');
			data.addColumn('number', 'Confirmados');
...
<body>
	<div class="destaque1">
		<h1>Catálogo Coletivo: Reservas - entidades requisitantes</h1>
	</div>
	<!-- Segmento Navegação topo -->
	<div class="col3-contentor">
		<div class="col3">
			<p class="fil">» <a href="../default.asp">Zona Pública</a> » <a href="admin.asp?id=2&mnres=2">Administração</a> » Reservas [entidades requisitantes]
			</p>
		</div>
	...
	<!-- Segmento Navegação topo - FIM -->
		...
		<h3>Reservas [entidades requisitantes]</h3>
			...
			<div id="painel">
				<div id="tab1" class="tabcontent">
					<br />
					<b>A - Pedidos [Total]:</b>
						<div id="chart_div" style="width:100%">A carregar dados... Aguarde um momento, por favor</div>
						<div>
						<br />
					<%
			if flag=1 then
			...
			%>
				...
				<div id="tab2" class="tabcontent">
					...
					<b>B - Pedidos totais vs confirmados: [Empréstimo interbibliotecas]</b>
						<div id="chart_div1" style="width:100%">A carregar dados... Aguarde um momento, por favor</div>
						<br />
						<div>
					<%
			if flag=1 then
