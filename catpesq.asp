...
<head>
   ...
   function SwapFilter() {
      $('#txtfilter').html('[Tipo='+$('#tdoc :selected').text()+' ; Formato='+$('#formato :selected').text()+' ; Por pag.='+$('#limites :selected').text()+ ($('#dp').val() !=''? ' ; AP='+ $('#dp').val():'') + ($("#truncatura").length > 0 ? ($("#truncatura").is(':checked') ? ' ; Exata=sim':' ; Exata=não'):'')+']');
   }   
