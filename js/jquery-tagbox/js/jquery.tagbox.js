...
(function(jQuery) {
  jQuery.fn.tagBox = function(options) {
    var defaults = {
      ...
      tagInputClassName: '',
      tagButtonClassName: '',
      tagButtonTitle: '../../imagens/picactions/icon_etiquetas.svg',
      ...
      removeTagText: "X",
      maxTags: -1,
      maxTagsErr: function(max_tags) { alert("Apenas pode adicionar um total de "+max_tags+" etiquetas!"); },
...
//  Hide Element
  ...
  //  Create DOM Elements
      ...
      var $tag_add_elem = (options.readonly) ? '' : jQuery('<a href="javascript:void(0)" ><img border="0" src="'+options.tagButtonTitle+'" alt="Adicionar etiqueta" title="Adicionar etiqueta"></a>');
  ...
  //  Methods
      ...
      function generate_tags_list(tags_list) {
        ...
        jQuery.each(tags_list, function(key, val) {
          if(val != "") {
            var remove_tag_link = (options.readonly) ? '' : '<a href="javascript:void(0)" class="'+options.className+'-remove '+options.className+'-remove-'+uuid+'" title="Remover etiqueta" rel="'+val+'">'+options.removeTagText+'</a>';
