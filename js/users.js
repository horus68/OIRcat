...
function getActionHistRes(base){
	 ...
	 var flag=true;
	 switch (x) {
	  case '': alert('ATENÇÃO. Selecione um critério!'); flag=false;break;
	  case 'X': exp= '$';break
...
function validadados(flag){
       ...
       var confirma=document.getElementById("confirma").value;
	    if (senha!=confirma)
        { msgconf= "As senhas não coincidem.\n";status=false; }
		...
		if (!stat_gps) { msgID += "As coordenadas GPS devem ser valores numéricos [ex: -9,02345].\n";status=false;}
...
function validadadosLeitor(){
       ...
       var msg="";var msgID="";
        ...
        {msg= (msg != "") ? msg+ ",Nº de LEITOR": msg+"Nº de LEITOR"; status=false; }
		    ...
		    var valor = parseInt(document.getElementById("v1").value);
            if (isNaN(valor)) 
            {msgID = "O campo N.º de leitor tem de ser numérico!\n";status=false;}
		}	
        if (document.getElementById("v2").value=="")
        {msg= (msg != "") ? msg+ ",NOME": msg+"NOME"; status=false; }
