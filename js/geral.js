...
function ShowExi(header,base,formato,mfn,biblio,ut,nut,bibleitor)
{
 ...
 var loc = "/rbcatalogo/cgi/www.exe/[in=pesqexi.in]?base="+base+"&ut="+ut+"&nut="+nut+"&formato="+formato+"&lim_inicio=1&limites=25&from="+mfn+"&to="+mfn+"&ent="+bibleitor+"&estab="+biblio;
...
function ShowRegs(mfn,exp,base,user,nuser,ex,count,sigla,bibleitor)
{
 ...
 var loc = "/rbcatalogo/cgi/www.exe/[in=pexiloc.in]?base="+base.toLowerCase()+"&expressao="+exp+"&lim_inicio=1&limites=1000&ut="+user+"&nut="+nuser+"&mfn="+mfn+"&nex="+ex+"&pos="+count+"&sigla="+sigla+"&ent="+bibleitor;
...
{
var janela =  "altpin"; 
var loc = "/rbcatalogo/admin/logleitor.asp?op=ap";
popup=window.open("",janela,"toolbar=no,location=no,directories=no,status=no,scrollbars=yes,resizable=no,menubar=no,width=390,height=290,left=150%,top=150%");
 ndoc=popup.document;
 ndoc.location.href=loc;
 if(window.focus) popup.focus();
}
...
function adcoment(id,mfn)
{
 ...
 var janela =  "coment"; 
 var loc = "/rbcatalogo/comentarios.asp?id="+id+"&mfn="+mfn;
...
function ShowMap(entidade,morada,concelho,lg,lt)
{
 ...
 var janela =  "map"; 
 var loc = "/rbcatalogo/mostra_mapa.asp?long="+lg+"&lat="+lt+"&entidade="+encode_utf8(entidade)+"&morada="+encode_utf8(morada)+"&concelho="+encode_utf8(concelho);
 ...
 function build_reservas(user,nuser)
{
 ...
 var janela =  "res"; 
 if (nuser != "")
 var loc = "/rbcatalogo/cgi/www.exe/[in=pesqres.in]?base=reservas&expressao=REQ "+nuser+"&lim_inicio=1&limites=99999&ut="+escape(user)+"&nut="+nuser;
 else
 var loc = "/rbcatalogo/cgi/www.exe/[in=pesqres.in]?base=reservas&expressao=REQ "+user+"&lim_inicio=1&limites=9999&ut="+user;
 ...
 function nova_reserva(titulo,autor,entidade,nut,sigla,cota,nreg,ex,user)
{
 ...
 var loc = "/rbcatalogo/cgi/www.exe/[in=newres.in]?base=reservas&titulo="+titulo+"&autor="+autor+"&entidade="+entidade+"&nut="+nuser+"&sigla="+sigla+"&cota="+cota+"&nreg="+nreg+"&ex="+ex+"&ut="+user;
...
function ValidaData(theForm)
{
  ...
  if (chkVal != "" && !(prsVal >= "1000" && prsVal <= "2050"))
  {
    alert("A data indicada deve estar entre 1000 e 2050.");
    theForm.DP.focus();
    return (false);
  }
}
...
//--------------------------------------
  ...
  if (part1=="")
     {  
       alert("ATENÇÃO! Não foi indicada uma etiqueta para pesquisa!\n\nPara mais informações consulte o guia do utilizador.");  
       return(false);
     } else 
     { 
...
function MailIt(base,formato,express,mfn1,limites)
{
 ...
 var loc = "/rbcatalogo/cgi/www.exe/[in="+infile+"]?base="+base+"&formato="+formato+"&expressao="+express+"&lim_inicio="+mfn1+"&limites="+limites;
...
function ver_his()
{
 ...
 popup=window.open("","historico","toolbar=no,location=no,directories=no,status=no,scrollbars=yes,resizable=yes,width=670,height=320,left=10,top=15");
 ndoc=popup.document;
 ndoc.location.href=loc;
